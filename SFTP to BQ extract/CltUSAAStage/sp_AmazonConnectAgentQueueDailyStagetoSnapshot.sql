CREATE OR REPLACE PROCEDURE CltUSAAStage.sp_AmazonConnectAgentQueueDailyStagetoSnapshot(IN iParentSystemId INT64)
BEGIN
	...
	SET vSql =
    FORMAT('''
		INSERT INTO `CltUSAAOds.AmazonConnect-AgentQueueDaily-FileSnapshot-%d` 
		(	
			ParentSystemId,
			SystemId,
			Cosa,
		...
		SELECT 
			...
			'' as InactiveReason
		FROM `CltUSAAStage.AmazonConnect-AgentQueueDaily-FileStage-%d` s;
