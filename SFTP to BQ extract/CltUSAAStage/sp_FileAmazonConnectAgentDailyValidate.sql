CREATE OR REPLACE PROCEDURE CltUSAAStage.sp_FileAmazonConnectAgentDailyValidate(IN iParentSystemId INT64, IN iSystemId INT64, OUT oMinDate DATETIME, OUT oMaxDate DATETIME, OUT oRejectedCount INT64)
BEGIN
    ...
    SET vSqlValidate =
        FORMAT('''
            UPDATE `CltUSAAStage.AmazonConnect-AgentDaily-FileStage-%d` t
            ...
            FROM (
                SELECT FileLoadAuditId, LineNum
                   ,[
					...
					THEN 'OnlineTime parse error' END
					,CASE
                ...
                FROM `CltUSAAStage.AmazonConnect-AgentDaily-FileStage-%d`
            ) s
