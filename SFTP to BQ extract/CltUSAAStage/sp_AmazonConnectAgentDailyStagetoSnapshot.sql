CREATE OR REPLACE PROCEDURE CltUSAAStage.sp_AmazonConnectAgentDailyStagetoSnapshot(IN iParentSystemId INT64)
BEGIN
	...
	SET vSql =
  	FORMAT('''
		INSERT INTO `CltUSAAOds.AmazonConnect-AgentDaily-FileSnapshot-%d`
		(	
			ParentSystemId,
			SystemId,
			Cosa,
		...
		SELECT 
			...
			'CltUSAAStage.sp_AmazonConnectAgentDailyStagetoSnapshot_%d' as CreateProcess,
			SESSION_USER() as UpdateBy,
			CURRENT_TIMESTAMP() as UpdateDate,
			'CltUSAAStage.sp_AmazonConnectAgentDailyStagetoSnapshot_%d' as UpdateProcess,
			s.FileLoadAuditId,
			s.LineNum,
			s.FileEffectiveDate,
			...
			'' as InactiveReason
		FROM `CltUSAAStage.AmazonConnect-AgentDaily-FileStage-%d`  s;
