CREATE OR REPLACE PROCEDURE CltUSAAStage.sp_AmazonConnectQueueRoutingDailyStagetoSnapshot(IN iParentSystemId INT64)
BEGIN
	...
	SET vSql =
    FORMAT('''
		INSERT INTO `CltUSAAOds.AmazonConnect-QueueRoutingDaily-FileSnapshot-%d` 
		(	
			ParentSystemId,
			SystemId,
			Cosa,
		...
		SELECT 
			...
			'CltUSAAStage.sp_AmazonConnectQueueRoutingDailyStagetoSnapshot_%d' as CreateProcess,
			SESSION_USER() as UpdateBy,
			CURRENT_TIMESTAMP() as UpdateDate,
			'CltUSAAStage.sp_AmazonConnectQueueRoutingDailyStagetoSnapshot_%d' as UpdateProcess,
			s.FileLoadAuditId,
			s.LineNum,
			s.FileEffectiveDate,
			...
			'' as InactiveReason
		FROM `CltUSAAStage.AmazonConnect-QueueRoutingDaily-FileStage-%d` s;
