CREATE OR REPLACE PROCEDURE CltUSAAStage.sp_FileAmazonConnectQueueRoutingIntervalValidate(IN iParentSystemId INT64, IN iSystemId INT64, OUT oMinDate DATETIME, OUT oMaxDate DATETIME, OUT oRejectedCount INT64)
OPTIONS(
  strict_mode=false)
BEGIN
    ...
    SET vSqlValidate =
        FORMAT('''
            UPDATE `CltUSAAStage.AmazonConnect-QueueRoutingInterval-FileStage-%d` t
            ...
            FROM (
                ...
                FROM `CltUSAAStage.AmazonConnect-QueueRoutingInterval-FileStage-%d`
            ) s
